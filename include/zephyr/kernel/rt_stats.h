...
static inline int k_thread_rt_stats_get(k_tid_t thread, struct k_thread_rt_stats *stats)
{
	ARG_UNUSED(thread);
	ARG_UNUSED(stats);
	return -ENOTSUP;
...
static inline int k_thread_rt_stats_reset(k_tid_t thread)
{
	ARG_UNUSED(thread);
	return -ENOTSUP;
}

static inline void k_thread_rt_stats_activation(k_tid_t thread)
{
	ARG_UNUSED(thread);
}

static inline void k_thread_rt_stats_deadline_miss(k_tid_t thread)
{
	ARG_UNUSED(thread);
}

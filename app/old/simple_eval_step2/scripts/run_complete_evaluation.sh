#!/bin/bash
# Master Script - complete evaluation workflow for simple_eval_step2
# Runs all tests and generates all graphs automatically
...
echo "════════════════════════════════════════════════════════════════════════════════"
echo "           RT Scheduler Evaluation - Complete Workflow"
echo "════════════════════════════════════════════════════════════════════════════════"
echo ""
...
run_tests() {
    ...
    if [ "$test_type" = "quick" ]; then
        print_info "Running quick demo (4 tests, ~5 minutes)"
        echo ""
        bash "$SCRIPT_DIR/quick_demo.sh"
    else
        print_info "Running full test suite (32 tests, ~40 minutes)"
        echo ""
        bash "$SCRIPT_DIR/run_all_tests.sh" "$RESULTS_DIR"
    fi
...
generate_graphs() {
    ...
    # Pass the results directory to the graphing script
    python3 "$SCRIPT_DIR/generate_graphs.py" "$RESULTS_DIR"
...
show_summary() {
    ...
    if [ -f "$APP_DIR/results/graphs/summary_report.txt" ]; then
        print_status "Summary report generated"
        echo ""
        echo "Quick preview:"
        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        head -n 30 "$APP_DIR/results/graphs/summary_report.txt"
        echo ""
    ...
    echo "  Summary report:  $APP_DIR/results/graphs/summary_report.txt"
    echo ""
    ...
    graph_count=$(find "$APP_DIR/results/graphs" -name "*.png" 2>/dev/null | wc -l)
...
show_menu() {
    ...
    echo ""
    echo "  1) Quick Demo    - Test 4 scheduler/workload/DW combinations (~5 minutes)"
    echo "  2) Full Suite    - Test all 32 combinations (~40 minutes)"
...
main() {
    ...
    if [ $# -eq 0 ]; then
        while true; do
           ...
           case $choice in
                ...
                2)
                    ...
                    ;;
                3)
                    if [ ! -d "$APP_DIR/results" ] || [ $(find "$APP_DIR/results" -maxdepth 1 -name "*.csv" | wc -l) -eq 0 ]; then
        ...
        case "$1" in
            ...
            help|--help|-h)
                ...
                echo "  full, -f      Run full test suite (32 tests)"
    ...
    echo "  • Review graphs in: $APP_DIR/results/graphs/"
    echo "  • Read summary report: $APP_DIR/results/graphs/summary_report.txt"
